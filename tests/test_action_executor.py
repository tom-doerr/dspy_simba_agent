...
def test_handle_think(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    xml_string = "<action type='think'><content> Pondering the meaning... </content></action>"
    action_element = ET.fromstring(xml_string)

    result = action_executor._handle_think(action_element)

    assert not result # Think action should return False
...
def test_handle_message(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    assert result # Message action should return True
    mock_io_handler.display_message.assert_called_once_with("Agent: Hello User!")

def test_handle_run_command_with_cwd(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    mock_io_handler.display_message.assert_not_called() # No error message expected

def test_handle_run_command_no_cwd(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    mock_io_handler.display_message.assert_not_called()

def test_handle_run_command_no_command(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    assert not result # Should return False if no command
...
def test_execute_action_valid(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    mock_io_handler.display_thought.assert_not_called()
    mock_io_handler.display_command_request.assert_not_called()

def test_execute_action_invalid_type(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    mock_io_handler.display_error.assert_not_called() # Should not be an error, just unknown

def test_execute_action_handler_error(action_executor: ActionExecutor, mock_io_handler: Mock):
    ...
    assert not result # Should return False on handler error
    mock_io_handler.display_message.assert_called_once_with("Agent: This will fail") # It was called
